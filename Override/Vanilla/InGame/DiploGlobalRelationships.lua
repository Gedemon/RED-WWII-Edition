			...
			if (g_pUsTeam:IsHasMet(iOtherTeam)) then
				...
				if (Players[g_iUs]:IsDenouncedPlayer(iOtherPlayer)) then
					...
					-- Normal denouncement
						...
						controlTable.DenounceLabel:SetText(Locale.ConvertTextKey("TXT_KEY_DIPLO_YOU_HAVE_DENOUNCED"));
					end
					bHasEntry = true;
				else
						...
						if (pThirdPlayer ~= nil and pThirdPlayer:IsAlive()) then
							...
							if (g_pUsTeam:IsHasMet(iThirdTeam) or iThirdPlayer == g_iUs) then
								...
								if (pOtherPlayer:IsDoF(iThirdPlayer)) then
									...
									local textControls = {};
									ContextPtr:BuildInstanceForControl("TextEntry", textControls, controlTable.PactStack);
									textControls.Text:LocalizeAndSetText("TXT_KEY_DIPLO_FRIENDS_WITH", thirdName);
								end
							end
						end
					end
					...
					if (iThirdPlayer ~= iOtherPlayer) then
						...
						if (pThirdPlayer ~= nil and pThirdPlayer:IsAlive()) then
							...
							if (g_pUsTeam:IsHasMet(iThirdTeam) or iThirdPlayer == g_iUs) then
								...
								-- Denounced
								if (pOtherPlayer:IsDenouncedPlayer(iThirdPlayer)) then
									bHasEntry = true;
									...
									-- Normal denouncement
										...
										textControls.Text:LocalizeAndSetText("TXT_KEY_DIPLO_DENOUNCED", thirdName);
									end
								end
							end
						end
