...
ScenarioCivilizations = {
	...
	[2] = "CIVILIZATION_RUSSIA",
	[3] = "CIVILIZATION_FRANCE",
	[4] = "CIVILIZATION_ROME",
	[5] = "CIVILIZATION_ENGLAND",
	[6] = "CIVILIZATION_AMERICA",
	[7] = "CIVILIZATION_JAPAN",
	[8] = "CIVILIZATION_CHINA",
}
...
scenarioMap = Modding.GetEvaluatedFilePath(myModId, myModVersion, "World_War_II_1941.Civ5Map");
...
Controls.StartButton:RegisterCallback(Mouse.eLClick, function()
	...
	if bHotseat then
		...
		if(playerIndex == nil) then
			...
			print("Random player index = " .. playerIndex )
		end	
	...
	elseif (not g_bDataLoaded ) then
		...
		modUserData.SetValue("RedLoading", "Earth1942")
...
function RefreshCheckBoxGameOptions()
	...
	g_REDCheckboxOptions = {

		{
			...
			Type = "EmbarkFromHarbor",
			Name = "Embark from harbors",
			ToolTip = "Units can only embark from tiles adjacents to a city with harbor",
			Disabled = false,
			GameOption = true,
			Checked = false,
			SortPriority = 1,
		...
		--[[{ -- A check must be added to all scripted events in scenario script file for this option...
...
function Initialize()
	...
	print("-------------------------------------")
	print ("R.E.D WW2 - Earth 1942 setup screen")
	...
	Controls.RedWW2Logo2:SetTextureAndResize( "1942background.dds" )
...
function SetPlayerIndex(playerIndex)
	...
	if(playerIndex ~= nil) then		
		...
		-- Sets Dawn of Man Quote
		Controls.Quote:LocalizeAndSetText("TXT_KEY_RED_EARTH1942_BRIEFING");		
    end
