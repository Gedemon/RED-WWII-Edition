-- RedDefinesProjects
-- Author: Gedemon
-- DateCreated: 5/6/2012 8:40:08 PM
--------------------------------------------------------------


-- Projects Type ID
--
PROJECT_ATTACK_AIRCRAFT = GameInfo.Projects.PROJECT_ATTACK_AIRCRAFT.ID
PROJECT_HEAVY_BOMBER = GameInfo.Projects.PROJECT_HEAVY_BOMBER.ID
-- FR
PROJECT_H39			= GameInfo.Projects.PROJECT_H39.ID
PROJECT_R40			= GameInfo.Projects.PROJECT_LIGHT_TANK_R40.ID
PROJECT_MB152		= GameInfo.Projects.PROJECT_MB152.ID
PROJECT_MS406		= GameInfo.Projects.PROJECT_MS406.ID
PROJECT_D520		= GameInfo.Projects.PROJECT_FIGHTER_D520.ID
-- RU
PROJECT_T34			= GameInfo.Projects.PROJECT_T34.ID
PROJECT_T34_76		= GameInfo.Projects.PROJECT_T34_76.ID
PROJECT_KV1         = GameInfo.Projects.PROJECT_KV1.ID
PROJECT_KV2			= GameInfo.Projects.PROJECT_KV2.ID
PROJECT_LAGG3		= GameInfo.Projects.PROJECT_LAGG3.ID
PROJECT_LA5			= GameInfo.Projects.PROJECT_LA5.ID
PROJECT_LA5_V2		= GameInfo.Projects.PROJECT_LA5_V2.ID
PROJECT_LA7			= GameInfo.Projects.PROJECT_LA7.ID
PROJECT_IL2			= GameInfo.Projects.PROJECT_IL2.ID
PROJECT_TU2			= GameInfo.Projects.PROJECT_BOMBER_TU2.ID
PROJECT_IL4			= GameInfo.Projects.PROJECT_IL4.ID
PROJECT_SOYUZ		= GameInfo.Projects.PROJECT_BATTLESHIP_SOYUZ.ID

--NEW
PROJECT_ARL_44		= GameInfo.Projects.PROJECT_ARL_44.ID
PROJECT_PANZER_V	= GameInfo.Projects.PROJECT_PANZER_V_PANTHER.ID
PROJECT_PANZER_VI	= GameInfo.Projects.PROJECT_PANZER_VI_TIGER.ID
PROJECT_IS_1		= GameInfo.Projects.PROJECT_IS_1.ID
PROJECT_IS_2		= GameInfo.Projects.PROJECT_IS_2.ID
PROJECT_M26 		= GameInfo.Projects.PROJECT_M26_PERSHING.ID
PROJECT_T34_85		= GameInfo.Projects.PROJECT_T34_85.ID
PROJECT_A15			= GameInfo.Projects.PROJECT_A15_CRUSADER.ID
PROJECT_A15_MKIII	= GameInfo.Projects.PROJECT_A15_CRUSADER_MKIII.ID
PROJECT_A13			= GameInfo.Projects.PROJECT_A13_COVENANTER.ID
PROJECT_A10			= GameInfo.Projects.PROJECT_A10_VALENTINE.ID
PROJECT_SHERMANIIIA = GameInfo.Projects.PROJECT_SHERMAN_IIIA.ID
PROJECT_SHERMAN_IV  = GameInfo.Projects.PROJECT_SHERMAN_IV.ID
PROJECT_JUMBO		= GameInfo.Projects.PROJECT_SHERMAN_JUMBO.ID
PROJECT_EASYEIGHT	= GameInfo.Projects.PROJECT_SHERMAN_EASYEIGHT.ID
PROJECT_B24			= GameInfo.Projects.PROJECT_B24.ID
PROJECT_LANCASTER	= GameInfo.Projects.PROJECT_LANCASTER.ID
PROJECT_YAK7		= GameInfo.Projects.PROJECT_YAK7.ID
PROJECT_MC202		= GameInfo.Projects.PROJECT_MC202.ID

-- GE
PROJECT_PANZER_II_L = GameInfo.Projects.PROJECT_LIGHT_TANK_PANZER_II_L.ID
PROJECT_PANZER_III  = GameInfo.Projects.PROJECT_PANZERIII.ID
PROJECT_PANZER_III_J = GameInfo.Projects.PROJECT_TANK_PANZER_III_J.ID
PROJECT_PANZER_IV   = GameInfo.Projects.PROJECT_PANZERIV.ID
PROJECT_PANZER_IV_G = GameInfo.Projects.PROJECT_TANK_PANZER_IV_G.ID
PROJECT_PANZER_IV_H = GameInfo.Projects.PROJECT_TANK_PANZER_IV_H.ID
PROJECT_BF109F		= GameInfo.Projects.PROJECT_BF109F.ID
PROJECT_BF109G		= GameInfo.Projects.PROJECT_BF109G.ID
PROJECT_FW190		= GameInfo.Projects.PROJECT_FW190.ID
PROJECT_BF110       = GameInfo.Projects.PROJECT_BF110.ID
PROJECT_BF110F4		= GameInfo.Projects.PROJECT_BF110F4.ID
PROJECT_JU88C		= GameInfo.Projects.PROJECT_JU88.ID
PROJECT_HE219		= GameInfo.Projects.PROJECT_HE219.ID
PROJECT_HE177		= GameInfo.Projects.PROJECT_HE177.ID
-- UK
PROJECT_TETRARCH    = GameInfo.Projects.PROJECT_TETRARCH.ID
PROJECT_MATILDAI    = GameInfo.Projects.PROJECT_MATILDAI.ID
PROJECT_MATILDAII   = GameInfo.Projects.PROJECT_MATILDAII.ID
PROJECT_CRUISER_III = GameInfo.Projects.PROJECT_CRUISERMKIII.ID
PROJECT_CRUISER_IV	= GameInfo.Projects.PROJECT_CRUISER_IV.ID
PROJECT_M4_FIREFLY	= GameInfo.Projects.PROJECT_M4_FIREFLY.ID
PROJECT_CHURCHILL	= GameInfo.Projects.PROJECT_CHURCHILL.ID
PROJECT_SPITFIRE_V	= GameInfo.Projects.PROJECT_SPITFIRE_V.ID
PROJECT_SPITFIRE_IX = GameInfo.Projects.PROJECT_SPITFIRE_IX.ID
PROJECT_WHIRLWIND	= GameInfo.Projects.PROJECT_WHIRLWIND.ID
PROJECT_BEAUFIGHTER = GameInfo.Projects.PROJECT_BEAUFIGHTER.ID
PROJECT_MOSQUITO	= GameInfo.Projects.PROJECT_BOMBER_MOSQUITO.ID
-- IT
PROJECT_L6_40		= GameInfo.Projects.PROJECT_L6040.ID
PROJECT_M11_39		= GameInfo.Projects.PROJECT_M1139.ID
PROJECT_M13_40		= GameInfo.Projects.PROJECT_M13_40.ID
PROJECT_M14_41		= GameInfo.Projects.PROJECT_M14_41.ID
PROJECT_M15_42		= GameInfo.Projects.PROJECT_M15_42.ID
PROJECT_P26_40		= GameInfo.Projects.PROJECT_P26_40.ID
PROJECT_G50		    = GameInfo.Projects.PROJECT_G50.ID
PROJECT_G55			= GameInfo.Projects.PROJECT_G55.ID
PROJECT_SM84		= GameInfo.Projects.PROJECT_SM84.ID
-- US
PROJECT_M3GRANT			    = GameInfo.Projects.PROJECT_M3GRANT.ID
PROJECT_M5STUART			= GameInfo.Projects.PROJECT_LIGHT_TANK_M5.ID
PROJECT_M4SHERMAN		    = GameInfo.Projects.PROJECT_M4SHERMAN.ID
PROJECT_M4A1SHERMANIIA		= GameInfo.Projects.PROJECT_SHERMAN_IIA.ID
PROJECT_SHERMAN_III			= GameInfo.Projects.PROJECT_SHERMAN_III.ID
PROJECT_P38					= GameInfo.Projects.PROJECT_P38.ID
PROJECT_P40					= GameInfo.Projects.PROJECT_P40.ID
PROJECT_P51					= GameInfo.Projects.PROJECT_P51.ID
PROJECT_B25					= GameInfo.Projects.PROJECT_B25.ID
PROJECT_B17					= GameInfo.Projects.PROJECT_B17.ID
-- JP
PROJECT_TYPE3				= GameInfo.Projects.PROJECT_TYPE3.ID
PROJECT_TYPE4				= GameInfo.Projects.PROJECT_TYPE4.ID
PROJECT_TYPE4_CHITO			= GameInfo.Projects.PROJECT_TYPE4_CHITO.ID
PROJECT_TYPE5				= GameInfo.Projects.PROJECT_TYPE5.ID

-- Military Operation
OPERATION_WESERUBUNG		= GameInfo.Projects.OPERATION_WESERUBUNG.ID
OPERATION_SEELOWE			= GameInfo.Projects.OPERATION_SEELOWE.ID
OPERATION_CHINA				= GameInfo.Projects.OPERATION_CHINA.ID
OPERATION_PACIFIC			= GameInfo.Projects.OPERATION_PACIFIC.ID
OPERATION_NORWAY			= GameInfo.Projects.OPERATION_NORWAY.ID
OPERATION_FALLGELB			= GameInfo.Projects.OPERATION_FALLGELB.ID
OPERATION_SONNENBLUME		= GameInfo.Projects.OPERATION_SONNENBLUME.ID
OPERATION_TWENTYFIVE		= GameInfo.Projects.OPERATION_TWENTYFIVE.ID
OPERATION_MARITA			= GameInfo.Projects.OPERATION_MARITA.ID
OPERATION_TORCH				= GameInfo.Projects.OPERATION_TORCH.ID


-- Available projects per nation
g_Major_Projects = {
	[FRANCE] = {		
		PROJECT_H39, PROJECT_R40, PROJECT_ARL_44,
		PROJECT_ATTACK_AIRCRAFT, PROJECT_MB152, PROJECT_MS406, PROJECT_D520,},
	[USSR] = {
		PROJECT_T34, PROJECT_T34_76,  PROJECT_KV1, PROJECT_KV2,	PROJECT_IS_1, PROJECT_IS_2, PROJECT_T34_85,
		PROJECT_LAGG3, PROJECT_LA5, PROJECT_LA5_V2, PROJECT_LA7, PROJECT_YAK7,
		PROJECT_IL2, PROJECT_IL4, PROJECT_HEAVY_BOMBER, PROJECT_TU2,
		PROJECT_SOYUZ,},
	[GERMANY] = {
		PROJECT_PANZER_II_L, PROJECT_PANZER_III, PROJECT_PANZER_III_J, PROJECT_PANZER_IV, PROJECT_PANZER_IV_G, PROJECT_PANZER_IV_H, PROJECT_PANZER_V, PROJECT_PANZER_VI,
		PROJECT_BF109F, PROJECT_BF109G, PROJECT_FW190, PROJECT_BF110, PROJECT_BF110F4, PROJECT_HE219, 
		PROJECT_JU88C, PROJECT_HE177,
		OPERATION_WESERUBUNG, OPERATION_SEELOWE, OPERATION_NORWAY, OPERATION_FALLGELB, OPERATION_SONNENBLUME, OPERATION_TWENTYFIVE, OPERATION_MARITA, },
	[ENGLAND] = {
		PROJECT_TETRARCH, PROJECT_MATILDAI, PROJECT_MATILDAII, PROJECT_CRUISER_III, PROJECT_CRUISER_IV, PROJECT_CHURCHILL, PROJECT_M4_FIREFLY, PROJECT_A10, PROJECT_A15,
		PROJECT_MOSQUITO, PROJECT_SPITFIRE_V, PROJECT_SPITFIRE_IX, 
		PROJECT_HEAVY_BOMBER, PROJECT_BEAUFIGHTER, PROJECT_WHIRLWIND, PROJECT_LANCASTER, },
	[ITALY] = {
		PROJECT_M13_40, PROJECT_M14_41, PROJECT_M15_42,  PROJECT_P26_40, PROJECT_M11_39, PROJECT_L6_40,
		PROJECT_G50, PROJECT_G55, PROJECT_MC202,
		PROJECT_ATTACK_AIRCRAFT, PROJECT_SM84, },
	[GREECE] = {
		PROJECT_ATTACK_AIRCRAFT,},
	[AMERICA] = {
		PROJECT_M5STUART, PROJECT_M3GRANT, PROJECT_M4SHERMAN, PROJECT_M4A1SHERMANIIA, PROJECT_SHERMAN_III, PROJECT_M26, PROJECT_SHERMANIIIA, PROJECT_SHERMAN_IV, PROJECT_JUMBO, PROJECT_EASYEIGHT,
		PROJECT_P51, PROJECT_P40, PROJECT_P38,
		PROJECT_B17, PROJECT_B25, PROJECT_B24,
		OPERATION_TORCH, },
	[JAPAN] = {
		PROJECT_TYPE3, PROJECT_TYPE4, PROJECT_TYPE4_CHITO, PROJECT_TYPE5,
		OPERATION_CHINA, OPERATION_PACIFIC, },
}

-- Projects Table
g_ProjectsTable = { 
	-- Buildings : need one of building types listed
	-- Projects : need all projects type listed
	-- Scenarios : available in all scenario listed, refer to ScenarioName of modUserData.SetValue("RedLoading", "ScenarioName") in setup lua file
	-- triggering events for project availability : (if not set, project is immediatly available)
		-- Type defined when the project is available
		-- xp = another unit must have gain this amount of XP during combat (XP from buildings are not counted)
		-- date = when starting date is reached (yyyymmdd), each turn the project have ProbPerTur % of chance of being made available
	
	[PROJECT_ATTACK_AIRCRAFT] =  {
		Buildings = {ACADEMY},
	},
	[PROJECT_HEAVY_BOMBER] =  {
		Buildings = {ACADEMY},
	},
	---------------------------------------------------------------------
	-- France
	---------------------------------------------------------------------
	[PROJECT_H39] =  {
		Buildings = {ACADEMY, FACTORY},
		Trigger = {Type = "xp", Value = 150, Reference = FR_H35 },
	},
	[PROJECT_R40] =  {
		Buildings = {ACADEMY, FACTORY},
		Trigger = {Type = "xp", Value = 150, Reference = FR_R35 },
	},
	[PROJECT_ARL_44] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19440101, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_D520] = {
		Buildings = {ACADEMY},
	},
	[PROJECT_MB152] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19370701, ProbPerTurn = 25},
	},
	[PROJECT_MS406] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19380101, ProbPerTurn = 15},
	},
	---------------------------------------------------------------------
	-- USSR
	---------------------------------------------------------------------
	[PROJECT_T34] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19400601, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_T34_76] =  {
		Buildings = {ACADEMY, LAND_FACTORY},	
		Trigger = {Type = "xp", Value = 100, Reference = RU_T34},
	},
	[PROJECT_T34_85] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "xp", Value = 300, Reference = RU_T34_76},
	},
	[PROJECT_KV1] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19390101, ProbPerTurn = 10},
		TopSecret = true,
	},
	[PROJECT_KV2] =  {
		Buildings = {ACADEMY, LAND_FACTORY},	
		Projects = {PROJECT_KV1},	-- available immediatly after KV-1 project is done
	},
	[PROJECT_IS_1] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19430101, ProbPerTurn = 20},
		TopSecret = true,
	},
	[PROJECT_IS_2] =  {
		Buildings = {ACADEMY},	
		Projects = {PROJECT_IS_1},	
		Trigger = {Type = "date", Value = 19430601, ProbPerTurn = 20},
		TopSecret = true,
	},
	[PROJECT_LAGG3] =  {
		Buildings = {ACADEMY},		
		Trigger = {Type = "date", Value = 19401101, ProbPerTurn = 15},
		TopSecret = true,
	},	
	[PROJECT_LA5] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},	
		Trigger = {Type = "xp", Value = 150, Reference = RU_LAGG3},
	},		
	[PROJECT_LA5_V2] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},	
		Trigger = {Type = "xp", Value = 250, Reference = RU_LA5},
	},
	[PROJECT_LA7] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},	
		Trigger = {Type = "xp", Value = 500, Reference = RU_LA5_V2},
	},	
	[PROJECT_YAK7] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19410901, ProbPerTurn = 35},
		TopSecret = true,
	},
	[PROJECT_IL2] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19401115, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_TU2] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19410101, ProbPerTurn = 10},
	},
	[PROJECT_IL4] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19400101, ProbPerTurn = 8},
	},
	[PROJECT_SOYUZ] =  {
		Buildings = {ACADEMY},
		TopSecret = true,
	},
	---------------------------------------------------------------------
	-- Germany
	---------------------------------------------------------------------
	[PROJECT_PANZER_II_L] =  {
		Buildings = {ACADEMY, FACTORY},
		Trigger = {Type = "xp", Value = 500, Reference = GE_PANZER_II },
	},
	[PROJECT_PANZER_III] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19360701, ProbPerTurn = 100},
		TopSecret = true,
	},
	[PROJECT_PANZER_III_J] =  {
		Buildings = {ACADEMY, LAND_FACTORY},
		Trigger = {Type = "xp", Value = 750, Reference = GE_PANZER_III },
	},
	[PROJECT_PANZER_IV] =  {
		Buildings = {ACADEMY},		
		Projects = {PROJECT_PANZER_III},	
		Trigger = {Type = "date", Value = 19370701, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_PANZER_IV_G] =  {
		Buildings = {ACADEMY, LAND_FACTORY},
		Trigger = {Type = "xp", Value = 250, Reference = GE_PANZER_IV },
	},
	[PROJECT_PANZER_IV_H] =  {
		Buildings = {ACADEMY, LAND_FACTORY},
		Trigger = {Type = "xp", Value = 750, Reference = GE_PANZER_IV_G },
	},
	[PROJECT_PANZER_V] =  {
		Buildings = {ACADEMY},	
		Projects = {PROJECT_PANZER_IV},
		Trigger = {Type = "date", Value = 19420901, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_PANZER_VI] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19420301, ProbPerTurn = 20},
		TopSecret = true,
	},
	[PROJECT_BF109F] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},
		Trigger = {Type = "xp", Value = 250, Reference = GE_BF109 },
	},
	[PROJECT_BF109G] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},
		Trigger = {Type = "xp", Value = 750, Reference = GE_BF109_F },
	},
	[PROJECT_BF110] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19360701, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_BF110F4] =  {
		Buildings = {ACADEMY, MEDIUM_AIR_FACTORY},
		Trigger = {Type = "xp", Value = 250, Reference = GE_BF110 },
	},
	[PROJECT_FW190] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19410501, ProbPerTurn = 20},
		TopSecret = true,
	},
	[PROJECT_JU88C] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19400101, ProbPerTurn = 15},
	},
	[PROJECT_HE219] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19430501, ProbPerTurn = 10},
		TopSecret = true,
	},
	[PROJECT_HE177] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19410601, ProbPerTurn = 10},
		TopSecret = true,
	},
	---------------------------------------------------------------------
	-- U.K.
	---------------------------------------------------------------------	
	[PROJECT_TETRARCH] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19380101, ProbPerTurn = 25},
	},	
	[PROJECT_MATILDAI] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19360701, ProbPerTurn = 15},
	},
	[PROJECT_MATILDAII] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19370101, ProbPerTurn = 10},
	},
	[PROJECT_A10] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19380210, ProbPerTurn = 35},
		TopSecret = true,
	},
	[PROJECT_A13] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19390101, ProbPerTurn = 20},
		TopSecret = true,
	},
	[PROJECT_A15] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19391201, ProbPerTurn = 35},
		TopSecret = true,
	},
	[PROJECT_A15_MKIII] =  {
		Buildings = {ACADEMY, LAND_FACTORY},	
		Trigger = {Type = "xp", Value = 75, Reference = UK_A15_CRUSADER},
	},
	[PROJECT_CRUISER_III] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19370101, ProbPerTurn = 20},
	},
	[PROJECT_CRUISER_IV] =  {
		Buildings = {ACADEMY, LAND_FACTORY},
		Trigger = {Type = "xp", Value = 50, Reference = UK_CRUISER_III },
	},
	[PROJECT_M4_FIREFLY] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19431201, ProbPerTurn = 15},
	},
	[PROJECT_CHURCHILL] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19410401, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_SPITFIRE_V] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},
		Trigger = {Type = "xp", Value = 250, Reference = UK_SPITFIRE },
	},
	[PROJECT_SPITFIRE_IX] =  {
		Buildings = {ACADEMY, SMALL_AIR_FACTORY},
		Trigger = {Type = "xp", Value = 750, Reference = UK_SPITFIRE_V },
	},
	[PROJECT_WHIRLWIND] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19391001, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_BEAUFIGHTER] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19400601, ProbPerTurn = 10},
		TopSecret = true,
	},
	[PROJECT_MOSQUITO] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19410101, ProbPerTurn = 10},
		TopSecret = true,
	},
	[PROJECT_LANCASTER] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19400101, ProbPerTurn = 35},
		TopSecret = true,
	},
	---------------------------------------------------------------------
	-- Italy
	---------------------------------------------------------------------
	[PROJECT_L6_40] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19390101, ProbPerTurn = 25},
	},
	[PROJECT_M11_39] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19390101, ProbPerTurn = 25},
	},
	[PROJECT_M13_40] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19390801, ProbPerTurn = 10},
	},
	[PROJECT_M14_41] =  {
		Buildings = {ACADEMY, LAND_FACTORY},
		Trigger = {Type = "xp", Value = 350, Reference = IT_M13_40 },
	},
	[PROJECT_M15_42] =  {
		Buildings = {ACADEMY, LAND_FACTORY},
		Trigger = {Type = "xp", Value = 700, Reference = IT_M14_41 },
	},
	[PROJECT_P26_40] =  {
		Buildings = {ACADEMY},
		Trigger = {Type = "date", Value = 19430401, ProbPerTurn = 10},
		TopSecret = true,
	},
	[PROJECT_G50] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19370201, ProbPerTurn = 25},
		TopSecret = true,
	},
	[PROJECT_G55] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19430301, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_MC202] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19410401, ProbPerTurn = 25},
		TopSecret = true,
	},
	[PROJECT_SM84] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19400601, ProbPerTurn = 15},
	},
	---------------------------------------------------------------------
	-- USA
	---------------------------------------------------------------------
	[PROJECT_M3GRANT] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19390101, ProbPerTurn = 15},
	},
	[PROJECT_M5STUART] =  {
		Buildings = {ACADEMY, FACTORY},
		Trigger = {Type = "xp", Value = 50, Reference = US_M3STUART },
	},
	[PROJECT_M4SHERMAN] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19400801, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_M4A1SHERMANIIA] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19420901, ProbPerTurn = 20},
	},
	[PROJECT_SHERMAN_III] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19430301, ProbPerTurn = 20},
	},
	[PROJECT_SHERMANIIIA] =  {
		Buildings = {ACADEMY, LAND_FACTORY},	
		Trigger = {Type = "xp", Value = 75, Reference = US_SHERMAN_III},
	},
	[PROJECT_SHERMAN_IV] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19440101, ProbPerTurn = 50},
		TopSecret = true,
	},
	[PROJECT_JUMBO] =  {
		Buildings = {ACADEMY, LAND_FACTORY},	
		Trigger = {Type = "xp", Value = 50, Reference = US_SHERMAN_IV},
		TopSecret = true,
	},
	[PROJECT_EASYEIGHT] =  {
		Buildings = {ACADEMY, LAND_FACTORY},	
		Trigger = {Type = "xp", Value = 50, Reference = US_SHERMAN_JUMBO},
		TopSecret = true,
	},
	[PROJECT_M26] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19440601, ProbPerTurn = 25},
		TopSecret = true,
	},
	[PROJECT_P40] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19380601, ProbPerTurn = 10},
	},
	[PROJECT_P38] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19390101, ProbPerTurn = 20},
		TopSecret = true,
	},
	[PROJECT_P51] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19430101, ProbPerTurn = 15},
		TopSecret = true,
	},
	[PROJECT_B17] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19360701, ProbPerTurn = 100},
	},
	[PROJECT_B24] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19391220, ProbPerTurn = 50},
		TopSecret = true,
	},
	[PROJECT_B25] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19400701, ProbPerTurn = 20},
	},
	---------------------------------------------------------------------
	-- Japan
	---------------------------------------------------------------------
	[PROJECT_TYPE3] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19440101, ProbPerTurn = 15},
	},
	[PROJECT_TYPE4] =  {
		Buildings = {FACTORY},	
		Trigger = {Type = "date", Value = 19440101, ProbPerTurn = 35},
	},
	[PROJECT_TYPE4_CHITO] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19420601, ProbPerTurn = 10},
	},
	[PROJECT_TYPE5] =  {
		Buildings = {ACADEMY},	
		Trigger = {Type = "date", Value = 19430301, ProbPerTurn = 10},
		TopSecret = true,
	},	
	
	---------------------------------------------------------------------
	-- MILITARY OPERATIONS	
	---------------------------------------------------------------------
	[OPERATION_WESERUBUNG] =  {
		Buildings = {BASE},	
		Scenarios = {"Euro1940", },	
		Trigger = {Type = "date", Value = 19400101, ProbPerTurn = 33},
		TopSecret = true,
	},

	[OPERATION_SEELOWE] =  {
		Buildings = {BASE},	
		Scenarios = {"Euro1940", },	
		Trigger = {Type = "date", Value = 19400401, ProbPerTurn = 20},
		TopSecret = true,
	},	
	
	[OPERATION_CHINA] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19370701, ProbPerTurn = 100},
		TopSecret = true,
	},
	[OPERATION_PACIFIC] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19411201, ProbPerTurn = 100},
		TopSecret = true,
	},
	[OPERATION_NORWAY] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19400401, ProbPerTurn = 100},
		TopSecret = true,
	},
	[OPERATION_MARITA] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19410101, ProbPerTurn = 100},
		TopSecret = true,
	},
	[OPERATION_TWENTYFIVE] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19410101, ProbPerTurn = 100},
		TopSecret = true,
	},
	[OPERATION_TWENTYFIVE] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19410101, ProbPerTurn = 100},
		TopSecret = true,
	},
	[OPERATION_SONNENBLUME] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19401110, ProbPerTurn = 20},
		TopSecret = true,
	},
	[OPERATION_TORCH] =  {
		Buildings = {BASE},	
		Scenarios = {"Earth1936", },	
		Trigger = {Type = "date", Value = 19420601, ProbPerTurn = 15},
		TopSecret = true,
	},
	
}

-- Units needing a project done before being build...
g_UnitsProject = {
	-- Tanks
	[FR_R40] = PROJECT_R40,					
	[FR_H39] = PROJECT_H39,			
	[IT_L6_40] = PROJECT_L6_40,			
	[IT_M11_39] = PROJECT_M11_39,			
	[IT_M13_40] = PROJECT_M13_40,			
	[IT_M14_41] = PROJECT_M14_41,			
	[IT_M15_42] = PROJECT_M15_42,			
	[IT_P26_40] = PROJECT_P26_40,				
	[RU_T34] = PROJECT_T34,				
	[RU_T34_76] = PROJECT_T34_76,			
	[RU_KV1] = PROJECT_KV1,
	[RU_KV2] = PROJECT_KV2,				
	[UK_CRUISER_III] = PROJECT_CRUISER_III,		
	[UK_CRUISER_IV] = PROJECT_CRUISER_IV,			
	[UK_TETRARCH] = PROJECT_TETRARCH,			
	[UK_MATILDA_I] = PROJECT_MATILDAI,			
	[UK_MATILDA_II] = PROJECT_MATILDAII,		
	[UK_M4_FIREFLY] = PROJECT_M4_FIREFLY,		
	[UK_CHURCHILL] = PROJECT_CHURCHILL,	
	[GE_PANZER_II_L] = PROJECT_PANZER_II_L,		
	[GE_PANZER_III] = PROJECT_PANZER_III,		
	[GE_PANZER_III_J] = PROJECT_PANZER_III_J,		
	[GE_PANZER_IV] = PROJECT_PANZER_IV,		
	[GE_PANZER_IV_G] = PROJECT_PANZER_IV_G,		
	[GE_PANZER_IV_H] = PROJECT_PANZER_IV_H,			
	[US_M3_GRANT] = PROJECT_M3GRANT,			
	[US_M5STUART] = PROJECT_M5STUART,			
	[US_SHERMAN] = PROJECT_M4SHERMAN,			
	[US_SHERMAN_IIA] = PROJECT_M4A1SHERMANIIA,			
	[US_SHERMAN_III] = PROJECT_SHERMAN_III,		
	[JP_TYPE4] = PROJECT_TYPE4,		
	[JP_TYPE3] = PROJECT_TYPE3,			
	[JP_TYPE4_CHI_TO] = PROJECT_TYPE4_CHITO,		
	[JP_TYPE5] = PROJECT_TYPE5,			
	-- Fighters
	[FR_MS406] = PROJECT_MS406,			
	[FR_MB152] = PROJECT_MB152,			
	[FR_D520] = PROJECT_D520,				
	[IT_G50] = PROJECT_G50,				
	[IT_G55] = PROJECT_G55,				
	[RU_LAGG3] = PROJECT_LAGG3,			
	[RU_LA5] = PROJECT_LA5,				
	[RU_LA5_V2] = PROJECT_LA5_V2,			
	[RU_LA7] = PROJECT_LA7,			
	[UK_SPITFIRE_V] = PROJECT_SPITFIRE_V,		
	[UK_SPITFIRE_IX] = PROJECT_SPITFIRE_IX,		
	[GE_BF109_F] = PROJECT_BF109F,			
	[GE_BF109_G] = PROJECT_BF109G,			
	[GE_FW190] = PROJECT_FW190,			
	[US_P40] = PROJECT_P40,				
	[US_P51] = PROJECT_P51,		
	-- Heavy Fighters			
	[UK_WHIRLWIND] = PROJECT_WHIRLWIND,		
	[UK_BEAUFIGHTER] = PROJECT_BEAUFIGHTER,		
	[GE_BF110] = PROJECT_BF110,			
	[GE_BF110_F4] = PROJECT_BF110F4,			
	[GE_JU88C] = PROJECT_JU88C,			
	[GE_HE219] = PROJECT_HE219,			
	[US_P38] = PROJECT_P38,			
	-- Attack Aircrafts
	[FR_LN401] = PROJECT_ATTACK_AIRCRAFT,			
	[IT_BA65] = PROJECT_ATTACK_AIRCRAFT,		
	[RU_IL2] = PROJECT_IL2,			
	[GR_FAIREY] = PROJECT_ATTACK_AIRCRAFT,			
	-- Fast Bombers		
	[UK_MOSQUITO] = PROJECT_MOSQUITO,			
	[RU_TU2] = PROJECT_TU2,			
	-- Medium Bombers
	[IT_SM84] = PROJECT_SM84,		
	[RU_IL4] = PROJECT_IL4,				
	[US_B25] = PROJECT_B25,				
	[CH_B25] = PROJECT_B25,
	-- Heavy Bombers				
	[RU_PE8] = PROJECT_HEAVY_BOMBER,				
	[GE_HE177] = PROJECT_HE177,			
	[UK_HALIFAX] = PROJECT_HEAVY_BOMBER,			
	[US_B17] = PROJECT_B17,
	-- Battleships
	[RU_BATTLESHIP] = PROJECT_SOYUZ,
	-- NEW
	[FR_ARL_44] = PROJECT_ARL_44,
	[GE_PANZER_V] = PROJECT_PANZER_V,	
	[GE_PANZER_VI] = PROJECT_PANZER_VI,
	[UK_A10_VALENTINE] = PROJECT_A10,
	[UK_A13_COVENANTER] = PROJECT_A13,	
	[UK_A15_CRUSADER] = PROJECT_A15,
	[UK_A15_CRUSADER_MKIII] = PROJECT_A15_MKIII,
	[UK_LANCASTER] = PROJECT_LANCASTER,
	[RU_YAK_7] = PROJECT_YAK7,
	[RU_IS_1] = PROJECT_IS_1,
	[RU_IS_2] = PROJECT_IS_2,
	[RU_T34_85] = PROJECT_T34_85,
	[US_SHERMAN_IIIA] = PROJECT_SHERMANIIIA,
	[US_SHERMAN_IV] = PROJECT_SHERMAN_IV,
	[US_SHERMAN_JUMBO] = PROJECT_JUMBO,
	[US_SHERMAN_EASYEIGHT] = PROJECT_EASYEIGHT,
	[US_M26_PERSHING] = PROJECT_M26,
	[US_B24] = PROJECT_B24,
	[IT_C202] = PROJECT_MC202,									
	
}
